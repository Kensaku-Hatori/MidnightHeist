...
#include "Component/Polygone/MeshInfoComponent.hpp"
#include "Component/Polygone/VertexRenderingComponent.hpp"
#include "Component/Polygone/DivisionComponent.hpp"
#include "Bace/fade.h"
...
void UpdateMeshLaserSystem::Update(entt::registry& Reg)
{
	...
	for (auto entity : view)
	{
		...
		if (!PlayerView.empty() && CManager::GetScene()->GetMode() == CScene::MODE_GAME)
		{
			...
			// 当たったら
			if (CollisionEntity(Reg, entity, playerEntity, ToPlayer) == true && ToPlayer < Distance && CFade::Instance().GetFade() == CFade::FADE_NONE)
			{
