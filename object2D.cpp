...
{
	m_Registry.emplace<Object2DComponent>(m_Entity);
	...
	Object2DComponent& my2DCom = m_Registry.get<Object2DComponent>(m_Entity);
	...
	// 頂点バッファをロック
	VERTEX_2D* pVtx = NULL;
	my2DCom.m_pVertex->Lock(0, 0, (void**)&pVtx, 0);
...
void CObject2D::Uninit()
{
	...
	// 頂点バッファの破棄
	if (my2DCom.m_pVertex != NULL)
	{
		my2DCom.m_pVertex->Release();
		my2DCom.m_pVertex = NULL;
	}
	// メモリの解放
	Release();
}
...
{
	Transform myTrans = m_Registry.get<Transform>(m_Entity);
	Object2DComponent& my2DCom = m_Registry.get<Object2DComponent>(m_Entity);
